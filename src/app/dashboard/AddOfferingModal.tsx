...
import { CourseDetails, FormData } from './utils/types';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-4 w-full max-w-md max-h-[80vh] overflow-y-auto relative">
        ...
        <h3 className="text-lg font-bold mb-3 text-gray-800">
          درس: {course.name} ({course.code})
        </h3>
        
        {error && (
          <div className="mb-3 p-2 text-sm text-red-700 bg-red-100 rounded-md">
            {error.includes("group_code_exists") 
              ? "این کد گروه از قبل برای این درس وجود دارد. لطفاً کد گروه دیگری انتخاب کنید."
              : error}
          </div>
        )}
        ...
        <form onSubmit={(e) => onSubmit(e, formData)} className="space-y-3">
          <div className="grid grid-cols-3 gap-2">
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-900">سال تحصیلی</label>
              <input
                type="text"
                name="year"
                value={formData.year}
            ...
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-900">ساعت آزمون</label>
              <input
                type="text"
                name="quiz_time"
            ...
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-900">تاریخ آزمون</label>
              <input
                type="text"
                name="examDate"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-900">استاد</label>
              <Select
            ...
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-900">کلاس</label>
              <Select
          ...
          {formData.times.map((time, index) => (
            <div key={index} className="border border-gray-200 rounded-md p-2 mt-2">
              <div className="flex justify-between items-center mb-1">
                <h4 className="text-xs font-medium text-gray-900">جلسه {index + 1}</h4>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-3 gap-1">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full px-1 py-1 text-xs text-gray-600 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all"
                    required
                  >
          ...
          <button
            type="button"
            onClick={addTimeSlot}
            className="w-full text-xs text-blue-600 hover:text-blue-800 py-1 border border-dashed border-gray-300 rounded-md mt-2"
          >
            + افزودن زمان جدید
          </button>
          ...
          <div className="pt-2 flex justify-center">
            <button
              type="submit"
              disabled={loading}
              className="w-40 bg-green-600 hover:bg-green-700 text-white py-1 px-3 rounded-md text-xs transition-colors disabled:opacity-50"
            >
